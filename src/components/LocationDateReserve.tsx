...
export default function LocationDateReserve ({onDateChange,onLocationChange}
    ...
    const[resrveDate,setReserveDate]=useState<Dayjs|null>(null)
    const[location,setLocation]=useState('BKK')

    return (
        <div className="bg-slate-100 rounded-1g space-x-5 space-y-2 w-fit px-10 py-5
        flex flex-row justify-center">
            ...
            <Select variant="standard" value={location}
            onChange={(e)=>{setLocation(e.target.value);onLocationChange(e.target.value)}}
            name="location" id="location" className="h-[2em] w-[200px]">
